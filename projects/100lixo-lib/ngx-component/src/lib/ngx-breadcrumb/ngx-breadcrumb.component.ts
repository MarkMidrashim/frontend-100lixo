...
export class NgxBreadcrumbComponent implements OnInit, OnDestroy {
  ...
  private breadcrumbs: BehaviorSubject<IBreadcrumb[]> = new BehaviorSubject<IBreadcrumb[]>([]);
  ...
  ngOnInit(): void {
    ...
    this._service.breadcrumb$
      ...
      .subscribe((res: IBreadcrumb[]) => {
        this.breadcrumbs.next(res);
        this.prepare();
      });
  }

  private prepare(): void {
    if (this.breadcrumbs.value?.length > 0) {
      const filterBreadcrumb = this.breadcrumbs.value.find((a: IBreadcrumb) => a.url === this._router.url);
      if (filterBreadcrumb) {
        this.breadcrumbs.next(this.breadcrumbs.value
          .sort((a: IBreadcrumb, b: IBreadcrumb) => a.position - b.position)
          .filter((a: IBreadcrumb) => a.position <= filterBreadcrumb.position)
        );

        this.lastBreadcrumb = this.breadcrumbs.value[this.breadcrumbs.value.length - 1];
